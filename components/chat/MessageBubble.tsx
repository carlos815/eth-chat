const MessageBubble = ({ name, message, isOwnMessage, lastOfGroup, firstOfGroup, timestamp }) => {
    ...
    return <div className={" flex flex-col  gap-y-1"}>
        {header}
        <div className={`rounded-2xl max-w-full p-4 text-white  max-w-3xl ${!isOwnMessage ? "self-start bg-neutral-600" : "self-end bg-gradient-to-l from-primary-400 to-primary-700 text-base"} ${!isOwnMessage && lastOfGroup && "rounded-bl-none"} ${isOwnMessage && lastOfGroup && "rounded-br-none"}`}>
